...
public:
	...
	VHACD::HullPlane GetPlaneEquation(const std::vector<VHACD::Vect3>& pointArray,
		bool& isValid) const;
	...
	const VHACD::Vect3& point) const
{
	...
	const VHACD::Vector3<VHACD::Googol> p0g = pointArray[m_index[0]];
	const VHACD::Vector3<VHACD::Googol> p1g = pointArray[m_index[1]];
	const VHACD::Vector3<VHACD::Googol> p2g = pointArray[m_index[2]];
	const VHACD::Vector3<VHACD::Googol> pointg = point;
	std::array<VHACD::Vector3<VHACD::Googol>, 3> exactMatrix = { p2g - p0g, p1g - p0g, pointg - p0g };
	return Determinant3x3(exactMatrix);
}

VHACD::HullPlane ConvexHullFace::GetPlaneEquation(const std::vector<VHACD::Vect3>& pointArray,
	bool& isvalid) const
{
	const VHACD::Vect3& p0 = pointArray[m_index[0]];
	const VHACD::Vect3& p1 = pointArray[m_index[1]];
	const VHACD::Vect3& p2 = pointArray[m_index[2]];
	VHACD::HullPlane plane(p0, p1, p2);
